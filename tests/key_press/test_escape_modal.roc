app [main!] {
    pf: platform "https://github.com/growthagent/basic-cli/releases/download/0.25.0/2Qj7ggHJdVV9jAspIjvskp_cUWvAyh7B9I-Ma_sY4zk.tar.br",
    playwright: "../../package/main.roc",
    spec: "https://github.com/niclas-ahden/roc-spec/releases/download/0.1.0/1gNyp2QAxomebg0_bZTY4WwD6WFyLjVl6TbC7Dr7AX8.tar.br",
}

import pf.Arg
import pf.Cmd
import pf.Env
import pf.Http

import playwright.Playwright {
    cmd_new: Cmd.new,
    cmd_args: Cmd.args,
    cmd_spawn_grouped!: Cmd.spawn_grouped!,
}

import spec.Assert
import spec.TestEnvironment {
    env_var!: Env.var!,
    http_send!: Http.send!,
    http_header: Http.header,
    pg_connect!: pg_connect_stub!,
    pg_cmd_new: pg_cmd_new_stub,
    pg_client_command!: pg_client_command_stub!,
}

pg_connect_stub! = |_| Err(NotImplemented)
pg_cmd_new_stub = |_| {}
pg_client_command_stub! = |_, _| Err(NotImplemented)

main! : List Arg.Arg => Result {} _
main! = |_args|
    TestEnvironment.with!(|worker_url|
        { browser, page } = Playwright.launch_page_with!({ browser_type: Chromium, headless: Bool.true, timeout: TimeoutMilliseconds(5000) })?

        Playwright.navigate!(page, "$(worker_url)/keyboard-modal")?

        # Verify initial state
        initial_status = Playwright.text_content!(page, "#status")?
        Assert.eq(initial_status, "Modal is closed") ? InitialStateShouldBeClosed

        # Open the modal
        Playwright.click!(page, "#open-modal")?

        # Wait for modal to become visible (JavaScript must have run)
        Playwright.wait_for!(page, "#modal", Visible)?

        # Verify modal is open
        status = Playwright.text_content!(page, "#status")?
        Assert.eq(status, "Modal is open") ? ModalShouldBeOpen

        # Press Escape to close the modal (targetless - page level)
        Playwright.key_press_targetless!(page, Escape, [])?

        # Wait for modal to become hidden
        Playwright.wait_for!(page, "#modal", Hidden)?

        # Verify modal is closed
        status_after = Playwright.text_content!(page, "#status")?
        Assert.eq(status_after, "Modal closed by Escape") ? ModalShouldBeClosed

        Playwright.close!(browser)
    )
