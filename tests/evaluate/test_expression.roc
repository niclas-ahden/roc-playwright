...
## Test: evaluate! returns string results
...
main! = |_args|
    TestEnvironment.with!(|worker_url|
        ...
        title_result = Playwright.evaluate!(page, "document.title")?
        Assert.eq(title_result, "Test Home Page") ? TitleShouldMatch

        # Evaluate expression that returns a string
        concat_result = Playwright.evaluate!(page, "'hello' + ' ' + 'world'")?
        Assert.eq(concat_result, "hello world") ? ConcatShouldMatch
