...
main! = |_args|
    TestEnvironment.with!(|worker_url|
        test_path = "/tmp/roc_playwright_test_upload.txt"
        File.write_utf8!("hello from test", test_path)?

        { browser, page } = Playwright.launch_page!(Chromium)?
        Playwright.navigate!(page, "$(worker_url)/file-upload")?

        Playwright.set_input_files!(page, "#any-file", Paths([test_path]))?
        ...
        Assert.eq(file_name, "roc_playwright_test_upload.txt") ? WrongFileName(file_name)
        Assert.eq(file_size, "15") ? WrongFileSize(file_size)
