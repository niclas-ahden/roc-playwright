...
main! = |_args| {
    ...
    page.set_input_files!("#single-file", Buffers([{
        name: "in_memory_test.zip",
        mime_type: "application/zip",
        buffer: List.repeat(0.U8, 512),
    }]))?

    file_name = page.evaluate!("document.querySelector('#single-file').files[0].name")?
    file_size = page.evaluate!("String(document.querySelector('#single-file').files[0].size)")?
