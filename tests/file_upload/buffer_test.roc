...
main! = |_args| {
    ...
    Playwright.set_input_files!(page, "#single-file", Buffers([{
        name: "in_memory_test.zip",
        mime_type: "application/zip",
        buffer: List.repeat(0.U8, 512),
    }]))?

    file_name = Playwright.evaluate!(page, "document.querySelector('#single-file').files[0].name")?
    file_size = Playwright.evaluate!(page, "String(document.querySelector('#single-file').files[0].size)")?

    Playwright.close!(browser)?

    Assert.eq(file_name, "in_memory_test.zip") ? |_| WrongFileName(file_name)
    Assert.eq(file_size, "512") ? |_| WrongFileSize(file_size)
