...
main! = |_args| {
    ...
    initial = page.text_content!("#status")?
    Assert.eq(initial, "Not hovered") ? |e| ShouldNotBeHoveredInitially(e)

    # Hover over the target element
    page.hover!("#hover-target")?

    # Verify hover was detected
    after_hover = page.text_content!("#status")?
    Assert.eq(after_hover, "Hovered") ? |e| ShouldBeHovered(e)
