...
main! = |_args| {
    url = worker_url!({})
    { browser, page } = Playwright.launch_page_with!(hooks, { browser_type: Chromium(DefaultChannel), headless: Bool.True, timeout: TimeoutMilliseconds(1000), args: [], has_touch: Bool.False, permissions: [] })?
    Playwright.navigate!(page, Url.to_str(Url.append_path(url, ["attributes-test"]).ok_or(url)))?

    # input_value! on a div should error
    result = Playwright.input_value!(page, "#no-href")
    _ = Assert.err(result) ? |e| ShouldErrorOnNonInputElement(e)
