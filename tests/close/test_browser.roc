...
main! = |_args|
    TestEnvironment.with!(|worker_url|
        ...
        Assert.eq(title_before, "Test Home Page") ? BrowserWorksBeforeClose
        ...
        result = Playwright.navigate!(page, worker_url)
        _ = Assert.err(result) ? BrowserDeadAfterClose

        # close! twice should fail (process is already dead)
        double_close_result = Playwright.close!(browser)
        _ = Assert.err(double_close_result) ? DoubleCloseFails
