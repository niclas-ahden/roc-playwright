...
main! = |_args|
    TestEnvironment.with!(|worker_url|
        ...
        Playwright.navigate!(page, "$(worker_url)/visibility-test")?

        # Element with visibility: hidden should not be visible
        invisible = Playwright.is_visible!(page, "#invisible-element")?
        Assert.eq(invisible, Bool.false) ? InvisibleElementShouldBeFalse
