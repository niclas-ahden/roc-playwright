// Test server for the Playwright test suite.
// (The old server was a basic-webserver Roc app. There is no basic-webserver
// for the new compiler yet, so node stands in. Routes and page contents are
// preserved from the old tests/server/main.roc.)
...
const click_test_page = `<!DOCTYPE html>
<html>
<head>
...
const visibility_test_page = `<!DOCTYPE html>
<html>
<head>
...
const attributes_test_page = `<!DOCTYPE html>
<html>
<head>
...
const no_title_page = `<!DOCTYPE html>
<html>
<head>
</head>
<body>
...
const delayed_element_page = `<!DOCTYPE html>
<html>
<head>
...
const disappearing_element_page = `<!DOCTYPE html>
<html>
<head>
...
const never_appears_page = `<!DOCTYPE html>
<html>
<head>
...
const bounding_box_test_page = `<!DOCTYPE html>
<html>
<head>
...
const touch_test_page = `<!DOCTYPE html>
<html>
<head>
...
const keyboard_modal_page = `<!DOCTYPE html>
<html>
<head>
...
const keyboard_form_page = `<!DOCTYPE html>
<html>
<head>
...
const keyboard_navigation_page = `<!DOCTYPE html>
<html>
<head>
...
const keyboard_select_page = `<!DOCTYPE html>
<html>
<head>
...
const hover_test_page = `<!DOCTYPE html>
<html>
<head>
...
const mouse_test_page = `<!DOCTYPE html>
<html>
<head>
...
const file_upload_page = `<!DOCTYPE html>
<html>
<head>
...
const checkbox_test_page = `<!DOCTYPE html>
<html>
<head>
    <title>Checkbox Test</title>
    <style>
        /* A custom-styled checkbox: Chromium's CDP click does not fire
           'change' on these, which is why set_checked! goes through JS. */
        #styled { appearance: none; width: 20px; height: 20px; border: 2px solid #333; }
        #styled:checked { background: #333; }
    </style>
</head>
<body>
    <h1>Checkbox Test</h1>
    <input type="checkbox" id="plain">
    <input type="checkbox" id="styled">
    <input type="checkbox" id="prechecked" checked>
    <input type="radio" name="choice" id="radio-a">
    <input type="radio" name="choice" id="radio-b">
    <p id="change-count">0</p>
...
// Text carrying every character class the driver has to JSON-escape on the
// way back: double quote, backslash, newline, tab, and non-ASCII.
const escapes_test_page = `<!DOCTYPE html>
<html>
<head>
    <title>He said "hi"</title>
</head>
<body>
    <h1 id="quoted">He said "hi"</h1>
    <h1 id="backslash">C:\\Users\\test</h1>
    <pre id="multiline">line one
line two</pre>
    <h1 id="unicode">café · naïve · 日本語</h1>
    <input id="quoted-input" value='a "quoted" value'>
    <a id="link" href='/path?q="x"&amp;r=y'>link</a>
    <!-- Content that mimics the wire format's own serialized-string ("s":)
         and null ("v":) markers, to catch a decoder that routes by content. -->
    <pre id="json-content">{"s": "sniff", "v": "bait"}</pre>
    <div id="json-attr" data-state='{"s": "sniff", "v": "bait"}'>attr carrier</div>
</body>
</html>`;

const scroll_test_page = `<!DOCTYPE html>
<html>
<head>
    <title>Scroll Test</title>
    <style>
        body { margin: 0; }
        #spacer { height: 3000px; background: linear-gradient(#fff, #999); }
        #pointer-area {
...
<body>
    <div id="pointer-area">Pointer area</div>
    <div id="spacer"></div>
    <p id="scroll-y">0</p>
    <p id="pointer-events">none</p>
    <script>
        // Deliberately not named scrollY: a global var of that name becomes
        // window.scrollY and would clobber the real scroll offset.
        var scrollOut = document.getElementById('scroll-y');
        window.addEventListener('scroll', function () {
            scrollOut.textContent = String(Math.round(window.scrollY));
        });

        // touch_swipe! drives real compositor touch input, so the pointer
        // event sequence below only fires for genuine touch gestures.
        var seen = [];
        var area = document.getElementById('pointer-area');
        var pointerEvents = document.getElementById('pointer-events');
        ['pointerdown', 'pointermove', 'pointerup', 'pointercancel'].forEach(function (type) {
            area.addEventListener(type, function (e) {
                if (e.pointerType === 'touch' && seen.indexOf(type) === -1) {
                    seen.push(type);
                    pointerEvents.textContent = seen.join(',');
                }
            });
        });
    </script>
...
const routes = {
  ...
  "/escapes-test": escapes_test_page,
  "/scroll-test": scroll_test_page,
  "/page1": page1,
  "/page2": page2,
  "/form": form_page,
  "/click-test": click_test_page,
  "/visibility-test": visibility_test_page,
  "/attributes-test": attributes_test_page,
  "/no-title": no_title_page,
  "/delayed-element": delayed_element_page,
  "/disappearing-element": disappearing_element_page,
  "/never-appears": never_appears_page,
  "/bounding-box-test": bounding_box_test_page,
  "/touch-test": touch_test_page,
  "/keyboard-modal": keyboard_modal_page,
  "/keyboard-form": keyboard_form_page,
  "/keyboard-navigation": keyboard_navigation_page,
  "/keyboard-select": keyboard_select_page,
